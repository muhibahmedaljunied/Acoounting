              ...
              <tbody v-if="delay_types && delay_types.length > 0">
                <tr v-for="(delay_type, index) in delay_types" :key="index">
                  ...
                  <td>
                    ...
                    <button
                      type="button"
                      @click="delete_delay_type(delay_type.id)"
...
export default {
  ...
  methods: {
    ...
    get_search(word_search) {
      this.axios
        .post(`/delay_typesearch`, { word_search: this.word_search })
        .then(({ data }) => {
          this.delay_types = data;

          // this.$root.logo = "Category";
        });
    },
    delete_delay_type(id) {
      this.axios
        .post(`delete_delay_type/${id}`)
        .then((response) => {

...
trait GeneralTrait
{
    use TemporaleTrait, StockTrait, StoreProductTrait, ConditionTrait, DetailsTrait, ReturnTrait;


    public function add_start($data,$discount=null)
    {
    ...
    public function payment_purchase($data, $id)
    {
    ...
    public function payment_sale($data, $id)
    {


        $ayment_status = 'pendding';
        $payment_info = $data['type'];
        if ($data['paid'] == 0) { $payment_status = 'pendding';}
        if ($data['paid'] != 0 && $data['remaining'] != 0) {$payment_status = 'Partially';}
        ($data['remaining'] == 1) ? $payment_info = $data['type'] : $payment_info = 'credit' ;$payment_status = 'paiding';
